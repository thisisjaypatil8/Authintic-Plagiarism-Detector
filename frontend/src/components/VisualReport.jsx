...
const VisualReport = ({ structuredText, analysisResult }) => {
    ...
    useEffect(() => {
        // Guard: only fetch once per analysisResult identity
        if (
            !analysisResult ||
            !analysisResult.flagged_sections ||
            analysisResult.flagged_sections.length === 0
        ) return;

        // Prevent duplicate calls (React StrictMode fires effects twice)
        ...
        const fetchOverallGuidance = async () => {
            ...
            try {
                ...
                if (abortController.signal.aborted) return; // ignore cancelled
        ...
        return () => {
            ...
            // Reset so StrictMode's second mount can re-fetch
